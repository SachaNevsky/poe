    ...
    return (
        <div className={`${className} mx-4`}>
            {acts.map((act) =>
                <button
                    key={`act${act}`}
                    className={`w-20 lg:w-32 text-start rounded ml-1 lg:ml-6 px-1 lg:px-4 py-1 lg:py-2 cursor-pointer text-base lg:text-2xl ${act === currentAct ? "bg-red-900" : "bg-yellow-900"}`}
